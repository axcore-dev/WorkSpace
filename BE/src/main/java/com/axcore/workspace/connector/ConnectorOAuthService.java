 ...
 *
 * <p>세 단계다. (1) {@link #authorizeUrl}: 제공자 동의 화면 주소를 만들어 준다. state 는 우리가 서명한다.
 ...
 *
 * <p>셋 다 <b>연동 관리 권한</b>({@code can_manage_integrations})이 필요하다. 회사 단위 연결이라 누가 연결하느냐가
 * 곧 회사가 무엇을 허락하느냐다. 소유자와 서버 운영자는 항상 가진다.
 *
 * <p>지금 제공자 구현은 구글 하나다. 슬랙·노션은 같은 세 단계에 제공자 클래스만 더한다.
 */
@Service
public class ConnectorOAuthService {
    ...
    public String authorizeUrl(JwtPrincipal principal, String slug, Instant now) {
        TenantContext ctx = open(principal);
        ...
        accounts.find(app.provider()).ifPresent(a -> scopes.addAll(a.scopes()));
    ...
    /**
     * 콜백. state 가 <b>지금 이 요청의 회사·사용자</b>와 맞아야 한다 — 남이 시작한 흐름의 code 를 내 회사에
     * 심거나, 내가 시작한 흐름을 남의 회사에 심는 것을 둘 다 막는다.
     */
    @Transactional
    public void complete(JwtPrincipal principal, String slug, String code, String state, Instant now) {
        ...
        accounts.upsert(
                ...
                email,
                ctx.userId());
        setFlag(slug, true, ctx);
        log.info("워크스페이스 {} 가 {} 를 연결했다 (제공자 {}, 사용자 {}, 앱 {})",
                ctx.workspaceId(), slug, app.provider(), ctx.userId(), claims.slug());
    }
    ...
    public void setEnabled(JwtPrincipal principal, String slug, boolean enabled) {
        ...
        if (enabled) {
            ...
            log.info("워크스페이스 {} 가 {} 를 켰다 (계정 {})", ctx.workspaceId(), slug, account.externalAccount());
        } else {
            log.info("워크스페이스 {} 가 {} 를 껐다. 등록은 남긴다", ctx.workspaceId(), slug);
        }
        setFlag(slug, enabled, ctx);
    }
    ...
    public void disconnect(JwtPrincipal principal, String slug) {
        TenantContext ctx = open(principal);
        ConnectorCatalog.App app = app(slug);
        jdbc.update("delete from connected_services where slug = ?", slug);

        boolean othersRegistered =
                ConnectorCatalog.appsOf(app.provider()).stream()
                        .filter(a -> !a.slug().equals(slug))
                        .anyMatch(a -> registered(a.slug()));
        if (othersRegistered) {
            log.info("워크스페이스 {} 가 {} 를 해제했다. 같은 계정을 쓰는 앱이 남아 토큰은 둔다", ctx.workspaceId(), slug);
            return;
        }
        accounts.find(app.provider())
                .ifPresent(
                        a -> {
                            ...
                            accounts.delete(a.provider());
                        });
        log.info("워크스페이스 {} 가 {} 를 해제했다. 마지막 앱이라 {} 계정도 지웠다", ctx.workspaceId(), slug, app.provider());
    }

    // ------------------------------------------------------------------ 내부

    private TenantContext open(JwtPrincipal principal) {
    ...
    private void setFlag(String slug, boolean on, TenantContext ctx) {
        jdbc.update(
                """
    ...
    private boolean registered(String slug) {
        Integer n = jdbc.queryForObject("select count(*) from connected_services where slug = ?", Integer.class, slug);
        return n != null && n > 0;
    }
}
