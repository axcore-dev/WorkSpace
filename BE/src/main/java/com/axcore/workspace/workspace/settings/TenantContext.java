 ...
 * @param owner       회사에 한 명인 소유자({@code roles.code = 'owner'})인가. 직급·권한 편집은 이 사람만
...
public record TenantContext(
    ...
    public static final String INTERNAL_ADMIN_ROLE = "internal_admin";

    /** 회사 설정(기능 · 연동)과 구성원을 바꿀 수 있어야 한다. */
     ...
     * 범위 검사는 직급 API 가 대상 직급을 알 때 한다(2단계). 여기서는 문 앞에서 둘 중 하나인지만 본다.
     */
    ...
    /** 구성원 초대·소속 변경. */
    public void requireInviter() {
        if (!admin && !canInvite) {
            throw new SettingsForbiddenException("구성원을 초대할 권한이 없습니다");
        }
    }
}
