 ...
 * </ol>
 *
 * <p><b>이메일을 확인된 것으로 취급한다.</b> 네이버 응답에 {@code email_verified} 가 없어서
 * 우리가 정해야 하는 값이고, {@code true} 로 둔다. 근거는 네이버가 연락처 이메일을 등록·변경할
 * 때 그 주소로 인증 메일을 보낸다는 것이다 — 확인을 마치지 않은 주소는 프로필에 실려 오지 않는다.
 *
 * <p><b>이 값이 무엇을 여는지 알고 두는 것이다.</b> {@link
 * com.axcore.workspace.user.service.SocialAccountLinker} 에서 두 갈래가 이 값에 달려 있다 —
 * 같은 주소를 쥔 <b>확인된 계정에 자동으로 연결</b>하는 것과, 주소를 쥔 <b>미확인 계정을
 * 밀어내는</b> 것이다. 앞쪽이 무겁다. 주소의 주인이 아닌 사람을 그 주소의 계정에 넣어 주는
 * 것이라, 위 근거가 틀리는 날 계정 탈취가 된다.
 *
 * <p><b>그래서 이 판단은 네이버의 동작에 기대고 있고, 그 사실을 여기 남긴다.</b> 네이버가 API
 * 문서로 보증한 값이 아니라 계정 설정 화면에서 관찰되는 동작이다. 연락처 이메일을 인증 없이
 * 바꿀 수 있게 되면 이 줄이 그대로 취약점이 되므로, 그때 {@code false} 로 되돌리고 아래 「대안」
 * 으로 옮겨야 한다.
 *
 * <p><b>대안</b> — 로그인한 사용자가 설정에서 직접 누르는 연결이다. 그 경로는 본인이 이미 인증된
 * 세션으로 증명하므로 제공자의 이메일 확인이 필요 없고, 이 판단 자체를 없앤다. 그 기능이 붙으면
 * 여기를 {@code false} 로 되돌려도 사용자가 잃는 것이 없다.
 *
 * <p>{@code false} 였을 때 무엇이 막혔는지는 남겨 둔다. 이미 이메일로 가입해 확인까지 마친
 * 사용자가 네이버 버튼으로 그 계정에 붙지 못했다({@link SocialLinkBlockedException}). 설정
 * 연결 기능이 없어 우회로도 없었고, 로그인 화면에 있는 버튼이 그 사용자에게는 영구히 동작하지
 * 않는 상태였다. 그것을 고치는 변경이다.
 *
...
public class NaverOAuthClient implements OAuthClient {
    ...
    public OAuthUserInfo fetchUserInfo(String code, String state) {
        ...
        return new OAuthUserInfo(
                ...
                profile.id(),
                profile.email(),
                // 네이버는 email_verified 를 주지 않아 우리가 정한다. 연락처 이메일은 등록·변경
                // 시 네이버가 인증 메일을 보내므로 확인된 것으로 본다. 이 한 줄이 기존 계정
                // 자동 연결을 여는 지점이다 — 근거와 되돌릴 조건은 위 클래스 주석에 있다.
                true,
                profile.displayName(),
