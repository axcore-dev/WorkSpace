...
| `DEMO_LOGIN_EMAIL` `DEMO_LOGIN_PASSWORD` | 시연 계정 자격 (BE 만 읽는다) | `INFRA/seed/seed-demo-workspace.sh` 가 만든 소유자(`<편지함>+demo-owner@…`)와 그때 쓴 비밀번호. 비우면 `POST /api/auth/demo-login` 이 404. **브라우저로 나가지 않는다**(#92). 바꾸면 BE 재시작만 필요하다 — FE 재빌드는 버튼 플래그를 바꿀 때만 |
