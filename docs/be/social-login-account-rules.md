...
| 상황 | Google | 네이버 |
| --- | --- | --- |
| 처음 가입 | 바로 진행 (`SELECT_WORKSPACE`) | 동일 |
| 이미 **확인된** 이메일 계정이 있음 | 연결됨 | 동일 |
| 이미 **미확인** 이메일 계정이 있음 | 밀어내고 새로 만듦 | 동일 |
| 두 번째 로그인 이후 | 동일 | 동일 (연결된 뒤로는 이메일을 보지 않는다) |

**두 제공자의 동작이 같아졌다.** 이전에는 네이버가 아래·가운데 두 칸에서 409 였고, 그래서 이미
이메일로 가입한 사용자는 네이버 버튼으로 그 계정에 붙을 수 없었다. 우회할 설정 연결 기능도 없어
로그인 화면의 버튼이 그 사용자에게는 영구히 동작하지 않았다. 사용자 눈에는 "구글은 되는데 네이버는
왜 안 되지" 로 보였다.
