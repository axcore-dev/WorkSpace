...
| --- | --- |
| 없음 | 계정 생성 + 확인 메일 |
| **확인된 계정, 비밀번호 있음** | `DuplicateEmailException` (409) |
| **확인된 계정, 비밀번호 없음**(소셜 전용) | 그 계정에 비밀번호·이름을 붙이고 **확인을 다시 받는다** (`User#completeEmailSignUp`) |
| **미확인 계정** | **밀어내고** 새로 만든다 |

소셜 전용 계정을 막지 않는 이유: 소셜로 먼저 들어온 사람이 이메일로도 가입하려는 흐름을
「이미 가입됨」으로 끊으면 갈 곳이 없다. 대신 확인 시각을 비우고 확인 메일을 다시 보낸다.
가입 요청은 주소만 알면 누구나 보낼 수 있지만, 확인 메일은 주소의 주인에게만 가고 확인 전에는
`SessionIssuer#nextStep` 이 회사 선택 앞에서 막는다. 원래 주인이 소셜로 로그인해도 같은 확인을
한 번 더 거치게 된다 — 그 마찰을 받아들인다.
...
| 항목 | 왜 필요한가 |
| --- | --- |
| ~~**계정 설정의 소셜 연결**~~ — **붙었다(2026-09-10).** `POST /api/auth/identities/{provider}` · `SocialAccountLinker#linkToCurrent`. 인증된 세션이 본인 증명이라 제공자의 이메일 확인을 보지 않는다. 그 제공자 계정이 다른 사용자에게 이미 연결돼 있으면 409 로 막고 그렇다고 알려 준다(가져오지 않는다). 해제는 `DELETE /api/auth/identities/{provider}` — 마지막 로그인 수단(비밀번호 없음 + 유일한 연동)은 409 | 이제 네이버 `emailVerified=true` 판단을 `false` 로 되돌릴 수 있는 조건이 갖춰졌다. 되돌릴지는 별도 결정이다 — 로그인 화면의 네이버 버튼으로 기존 이메일 계정에 자동 연결되는 동작이 함께 사라진다 |
