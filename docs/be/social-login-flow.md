...
| `email_verified` | 준다 | **주지 않는다 → 우리가 `true` 로 채운다** (보안 판단 전용) |
...
| 없음 | — | 계정을 새로 만들고 **우리 확인 메일을 보낸다** |
| **확인된 계정** | 확인해 줌 | 그 계정에 연결한다 (이미 확인된 계정이라 메일 없음) |
| **확인된 계정** | 안 해 줌 | **409** — 허용하면 계정 탈취가 된다 |
| **미확인 계정** | 확인해 줌 | **밀어내고** 새로 만든다 + 확인 메일 |
| **미확인 계정** | 안 해 줌 | **409** — 어느 쪽도 주인이라는 증거가 없다 |

**「제공자 확인」 은 새 계정을 확인 처리할지에 쓰지 않는다.** 위 표의 보안 판단(연결·밀어내기)
전용이다. 새 계정은 제공자가 무엇을 말했든 우리가 확인 메일로 직접 증명한다 — 그래서 소셜
가입도 `EMAIL_VERIFICATION_REQUIRED` 로 시작한다.

밀어내기의 근거와 대가, 그리고 위 분리의 이유는 [계정 규칙 문서](./social-login-account-rules.md)에
있다.
