...
모델이 부를 수 있는 도구는 전부 레지스트리를 거친다. 사내 데이터 도구(`workspace_data`)와 구글 커넥터 도구가 여기 올라 있다.
...
## 업무 데이터 조회 — 온톨로지 `lib/ai/ontology.ts`

AI 가 조회할 수 있는 회사 데이터는 **개념** 단위로 온톨로지에 적는다. 개념마다 사람이 쓰는 말(동의어) · 속성 · 관계 ·
파생값이면 계산식 · 값을 가져오는 `load` 가 있다. 도구는 `workspace_data` 하나고(`server/data-tools.ts`), 도구 설명을
이 표에서 매 턴 생성한다. 조회마다 도구를 두지 않는 이유는 도구 백 개면 모델이 고르지 못하기 때문이다.

- **어휘 번역.** 사용자의 "제품 · 부품 · 자재" 는 `item`, "미매핑" 은 `bom_line.itemCode` 가 빈 것이다. 이 표가 없을 때
  모델은 업로드 문서에서 "미매핑 제품 리스트" 라는 문자열을 찾았다(2026-09-14).
- **파생값은 AI 서버가 센다.** 현재 재고(`stock`)는 화면과 같은 `stockOf`, 미매핑 BOM(`bom_line`)은 화면과 같은
  `liveLatest`(도면번호마다 최신 리비전 · 폐기 제외) 로 만든다. BE 는 원본만 주고 계산은 FE 함수 한 곳이다.
- **서버가 거른다.** `filter`(eq · ne · contains · is_null · not_null · gt · lt, AND) 를 적용하고 `limit`(기본 40) 개만
  돌려준다. `total` 은 거른 뒤 전체 건수라 "몇 건" 은 이 값으로 답한다. 도구 출력 4,000자 상한에 목록이 잘리지 않게 하는 장치다.
- **권한 두 겹.** 개념의 `tab` 을 가진 사람에게만 보이고(`conceptsFor`), 실제 GET 은 사용자 토큰으로 BE 를 불러 BE 가
  다시 막는다.
...
- **연동 화면에도 보인다.** `MES_DB_HOST` 가 있으면 설정 › 워크스페이스 › 연동 「외부 시스템」 에 「생산 MES」 한 줄이 붙는다
  (`ConnectorService.systems`, id 0). 상태 배지는 열 때마다 MES DB 에 실제로 붙어 본 결과다(정상 · 끊김).
...
- **접속 정보가 없으면 꺼진 채로 뜬다.** `MES_DB_HOST` 가 비면 MES 조회만 503(`CONNECTOR_UNAVAILABLE`)이고 다른 기능은
  그대로다. 부팅 때 MES 에 붙어 보지 않으므로 그쪽이 죽어 있어도 서버는 뜬다.
...
BE `.env` 값(전부 선택. 비면 MES 조회만 꺼진다):

| 변수 | 뜻 | 기본값 |
| --- | --- | --- |
| `MES_DB_HOST` `MES_DB_PORT` `MES_DB_NAME` | 고객 MES DB. Supabase 는 Session pooler(IPv4, 5432) 주소 | 없음 · `5432` · `postgres` |
| `MES_DB_USER` `MES_DB_PASSWORD` | SELECT 만 가진 롤. Supabase 풀러는 `롤.프로젝트ID` 형식 | 없음 |
| `MES_DB_SSLMODE` | | `require` |
...
- (완료 2026-09-14) 사내 데이터 도구 — 온톨로지(`lib/ai/ontology.ts`) 기반 `workspace_data`. 남은 것: 회사마다 다른 말을
