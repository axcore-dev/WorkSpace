...
> **⚠ 이 문서는 `docs/db/schema-draft-v2.md`로 대체되었습니다.**
> v1은 `workspace_id` 컬럼 + RLS 기반 **공유 스키마** 테넌시이고, v2는 회사 단위
> **스키마 격리**로 바뀌었습니다(`organizations`→`workspaces` 2계층도 회사 1계층으로 합쳐짐).
> 스키마 이름은 워크스페이스 PK 기반(`ax_00001`), 테넌트 식별 키는 사업자번호입니다.
> 테넌시·테이블 배치는 v2를 따르고, 이 문서는 화면 근거와 업무 흐름 서술을 참조할 때만 봅니다.
