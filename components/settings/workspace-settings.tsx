...
import { AiBadge, Badge, Button, Card, FIELD, SectionHeader, Toggle } from "@/components/ui";
import { MODULES } from "@/data/modules";
import { CONNECTOR_TYPES, CONNECTORS, EXTERNAL_SERVICES, SYNC_RULES } from "@/data/org";
  ...
  return (
    ...
    >
      <div className="space-y-4 p-5">
        <div>
          <label htmlFor="sys-name" className="mb-1.5 block text-sm font-medium text-slate-700">
          ...
          <input
            ...
            className={FIELD}
          />
          <p className="mt-1.5 text-xs text-slate-400">목록에 표시될 이름이에요. 용도를 알 수 있게 지어 주세요.</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label htmlFor="sys-type" className="mb-1.5 block text-sm font-medium text-slate-700">
              시스템 유형
            </label>
            <select id="sys-type" value={type} onChange={(e) => setType(e.target.value)} className={`${FIELD} cursor-pointer`}>
          ...
          <div>
            <label htmlFor="sys-system" className="mb-1.5 block text-sm font-medium text-slate-700">
              실제 시스템 명
            </label>
            <input
              id="sys-system"
        ...
        <div>
          <label htmlFor="sys-endpoint" className="mb-1.5 block text-sm font-medium text-slate-700">
            연결 주소 (엔드포인트)
          </label>
          <input
            id="sys-endpoint"
        ...
        <div>
          <label htmlFor="sys-key" className="mb-1.5 block text-sm font-medium text-slate-700">
            인증 키 (API Key)
          </label>
          <input
            id="sys-key"
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        ...
        <ul className="divide-y divide-slate-100">
          {connectors.map((c, i) => (
            <li key={`${c.name}-${i}`} className="flex flex-wrap items-center gap-3 py-3.5">
              <div className="min-w-0 flex-1">
                <p className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                  {c.name}
                  <span className="text-xs font-normal text-slate-400">{c.type}</span>
                </p>
                <p className="mt-0.5 truncate text-xs text-slate-400">{c.system}</p>
              </div>
              <Badge tone={c.status.tone}>{c.status.badge}</Badge>
              <Button variant="ghost" size="sm" onClick={() => openSystemModal(c)}>
                설정
              </Button>
            </li>
          ))}
      ...
      <Card>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {connectedServices.map((svc) => (
            <div key={svc.id} className="rounded-lg border border-slate-200 p-4">
              ...
              {svc.account && <p className="mt-2 truncate font-mono text-[11px] text-slate-400">{svc.account}</p>}
              <Button variant="secondary" size="sm" className="mt-3 w-full" onClick={() => disconnectService(svc.id)}>
                연결 해제
              </Button>
            </div>
          ))}
        ...
        {connectedServices.length === 0 && (
          <p className="py-8 text-center text-sm text-slate-500">
            연결된 서비스가 없어요. 우측 상단 &lsquo;커넥터 연결&rsquo;에서 추가해 주세요.
          </p>
        )}
      </Card>
      ...
      />
      <ConnectorModal open={connectorLibOpen} onClose={() => setConnectorLibOpen(false)} />
    </div>
  );
}
