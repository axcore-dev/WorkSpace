...
- 협업자 계정으로 실행 중이면 기존대로 알아서 진행한다(커밋 포함).
- 이 규칙은 `.claude/hooks/owner-confirm.sh` + `.claude/settings.local.json`의 PreToolUse 훅으로도 강제된다. 판별 기준은 `git config user.email` / `git config user.name` / OS 계정명이다.
- **주의**: 위 값들은 누구나 바꿀 수 있으므로 이 장치는 보안 경계가 아니다. 의도치 않은 자동 수정·자동 커밋을 막기 위한 실수 방지용이다.
