/**
 * 역할 순수 로직 — 부서별 묶기, 요약 문자열, 삭제 가능 판정, 구성원 수.
 *
...
/**
 * 부서별로 역할을 묶는다 — 화면 왼쪽 목록이 이 순서로 그린다.
 *
 * 부서가 없는 역할(소유자)이 **맨 위**에 온다. 그 다음이 `depts` 순서고, 역할이 없는
 * 부서도 남긴다 — 부서를 만들고 역할을 아직 안 만든 상태가 보여야 「역할 만들기」를
 * 어디에 눌러야 하는지 안다.
 *
 * `depts`에 없는 부서에 붙은 역할은 목록 끝에 따로 모은다. 버리면 화면에서 사라져서
 * 지울 수도 고칠 수도 없는 역할이 된다.
 */
