...
`Modal size="screen"` 은 **전폭 시트** — 화면을 덮는 편집 화면(헤더 · 푸터 고정, 본문 스크롤). 열이 많은 폼(발주서 작성: 라인 8열)이 팝업 폭에 갇혀 라인 0개에도 가로 스크롤이 생기던 자리에 쓴다(2026-09-11). 본문 폭 상한은 페이지 규칙대로 시트가 아니라 안의 블록이 진다(편집기 `max-w-[1400px]`). 닫기는 헤더 × 하나 — 푸터에 [닫기] 를 또 두면 확인 다이얼로그까지 「닫기」 가 넷이 된다. **머리 · 본문 · 바닥은 같은 기둥 클래스 `SCREEN_COLUMN`** 을 써서 넓은 화면에서도 좌우 끝이 한 줄에 선다. `Modal` 은 `document.body` 로 포털한다 — 부모에 transform 이 남아 있으면(`.fade-in`) `fixed` 가 그 부모에 갇힌다(2026-09-14). 발주서 작성의 요청자는 로그인한 구성원 이름이 자동으로 들어가는 읽기 전용 칸이다. 서식 토글 옆에는 두 서식이 실제로 찍는 열을 칩으로 나란히 보인다(「단위는 안 찍혀요」 같은 부정문 대신 무엇이 나가는지), 요약 줄(문서 N장 · 발주처별 라인 수)은 없다 — 바닥 「출력 · N장」 이 말한다. 라인은 표가 아니라 격자다: xl 이상 한 줄(9열), md 두 줄(6열), 그 아래 두 열 — `min-w` 표는 폭에 맞춰 줄을 바꾸지 못한다.
