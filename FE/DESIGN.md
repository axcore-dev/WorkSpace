...
| 설정 탭 | 표형(품목 · 거래처)은 카드 + 행 클릭 → 팝업(저장 시 자동 닫힘 + 「저장했어요」. **[저장] 은 바꾼 게 있을 때만 활성** — 수정됐는지가 버튼 상태로 보인다), 값형(안전 기준 · 문서 규칙)은 `SettingsSection` + 행마다 [수정] → 그 자리에서 고치고 [저장]. 태그처럼 작은 값 목록은 수정 모드 없이 인라인(Notion 의 다중 선택 값 편집) |
