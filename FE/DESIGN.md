...
캡이 달라(주요 정보는 캡 없음 · 모듈·설정은 1280) 화면을 옮길 때 제목의 왼쪽 x가 272↔512로 튀었고,
...
행 목록은 `max-w-3xl`을 자기 클래스에 갖는다(`SettingsRows`). 읽는 글이 주인 화면도 마찬가지다 —
AI대화 본문은 `max-w-3xl`을 유지한다(**AI대화는 이 규칙의 예외 화면이다**: 자체 셸을 쓰고 아우터가 없다).
