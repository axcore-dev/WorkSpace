...
| 행 펼침 | `DataTable` `expandedRow`/`renderExpanded` — 모달이 아니라 행 아래로. 패널 안의 primary 는 화면에 하나(「입고 등록」 → 편집 중엔 「저장」). **마감(「검수 완료」)은 secondary** — 잔량을 전부 취소로 만드는 길이 가장 눈에 띄는 버튼이면 안 된다. 편집 폼은 `<form>` 이라 Enter 가 저장이고, 첫 열린 라인의 수량 칸에 포커스가 선다. 초과 입고는 「잔량 N EA보다 M 많아요」로 사실을 먼저 말한다. 편집 중 다른 행을 누르면 「입력한 내용을 저장하지 않고 나가시겠습니까?」 [계속 입력] [저장하지 않고 나가기] |
...
| 검색 | 다섯 표가 `lib/search.ts` `matchesQuery` 하나로 걸러진다 — 공백 토큰 AND, 대소문자 · 공백 · 하이픈 · 점 무시(「PO 2607」 ≈ 「PO-2607-0021」, 「guide post」 ≈ 「GUIDE POST」), 토큰은 어느 필드에 맞아도 됨(「볼트 M8」 = 품명 + 규격). 발주 표는 라인 품명 · 호칭 · 규격까지 본다. 결과 0건이면 표 안에 `DataTable` `emptyText` 한 줄(「검색 결과가 없어요」), 「발주 전」 줄도 같은 검색어로 걸러진다. 플레이스홀더는 실제로 보는 필드를 적는다 |
