...
운영자 콘솔(`app/(admin)/layout.tsx`): 고객 앱(`AppShell`)과 크롬을 분리한다. **좌측 내비**(대시보드 / 워크스페이스 / 사용량·요금) + 넓은 본문(`max-w-6xl`), 하단에 로그인 운영자 정보. `lg` 미만에서는 내비가 상단으로 접힌다. 고객 화면의 브랜드 패널·사이드바를 쓰지 않는 이유는 메뉴·권한·데이터가 전혀 다르기 때문이다. 폭·간격·접기 규칙은 **「사이드바」 절**을 따른다(고객 앱과 같은 토큰).
...
| 층 | 요소 | 우리 적용 |
| --- | --- | --- |
| 1 | 브랜드 로고 + **접기/펼치기 토글** | 둘 다 있음. 토글은 로고 옆 |
