...
운영자 콘솔(`app/(admin)/layout.tsx`): 고객 앱(`AppShell`)과 크롬을 분리한다. **좌측 내비**(대시보드 / 워크스페이스 / 사용량·요금) + 넓은 본문(`main`에 `max-width` 제약 없음 — 필요한 화면은 개별적으로 `max-w-*`를 준다), 하단에 로그인 운영자 정보. `lg` 미만에서는 내비가 상단으로 접힌다. 고객 화면의 브랜드 패널·사이드바를 쓰지 않는 이유는 메뉴·권한·데이터가 전혀 다르기 때문이다. 폭·간격·접기 규칙은 **「사이드바」 절**을 따른다(고객 앱과 같은 토큰).
...
| 요소 | 결정 | 이유 |
| --- | --- | --- |
| **상태 표시기(배지)** | **쓰지 않는다** | 이 문서 「원칙」과 충돌한다 — *"상태/심각도는 옅은 색 텍스트로만 구분한다. 배경 채움·알약(pill) 배지를 쓰지 않는다."* 숫자 없는 빨간 점은 몇 건인지도 알려주지 않고, 숫자 배지는 대시보드에 이미 있는 값을 두 군데 두는 것이다 |
