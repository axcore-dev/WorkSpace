...
- **예외 — 재고 증감 부호**(2026-09-14, 입출고 이력 레퍼런스 요청). 이력의 수량 칸만 늘면 `text-red-600` · 줄면 `text-blue-600` + semibold(`DataTable` 셀 `{ delta }`). 증권 앱 관례를 따른 것이라 이 칸에서만 빨강이 「경고」, 파랑이 「누를 수 있음」이 아니다. 불합격 입고처럼 재고에 안 들어간 줄(내림 행)은 색을 지운다. 금액 · 다른 수량 칸으로 넓히지 않는다.
...
- **숫자 표기 — 쉼표는 세는 값에만**(2026-09-14). 수량 · 재고 · 안전 재고 · 부족 · 증감은 천 단위 쉼표(`lib/format.ts` `formatQty` · `formatDelta`, ko-KR). 코드 · 관리번호 · 규격 글자(`1500-80-MH`) · 날짜 · 입력 칸 · 내보내기 CSV 에는 붙이지 않는다 — 이름처럼 읽는 숫자이거나, CSV 에 쉼표가 들어가면 엑셀이 글자로 읽는다. 표의 음수는 ASCII `-`(복사해 붙여도 숫자로 읽힌다).
...
### 높이 사다리 — 36 과 32 두 단(2026-09-14)

한 줄에 32 · 26 · 28px 이 섞여 있었다(검색 아이콘 · 간단/상세 토글 · 「발주서 만들기」, 토글 글씨 12px). 컨트롤 높이는 두 단만 쓴다.

| 단 | 자리 | 컴포넌트 |
| --- | --- | --- |
| **36** | 페이지 머리 · 카드 머리 도구 줄 · 폼 바닥 · 설정 행 | `Button` md · `Segmented` md(기본, 14px) · 아이콘 버튼 `h-9 w-9`(아이콘 16) · `FIELD_MD` |
| **32** | 표 행 안 · 펼침 패널 안 · 편집기 라인 | `Button` sm(13px) · `Segmented size="sm"`(13px) · `FIELD_SM` |

- 28px 버튼 · 26px 토글은 없다. 12px 글씨의 컨트롤을 새로 만들지 않는다.
- 버튼 높이는 `min-h` 로 잡는다 — 좁은 폭에서 두 줄로 접히면 늘어나야 잘리지 않는다. 입력 칸은 한 줄이라 `h` 로 못 박는다(`FIELD_SM` 절).
...
- **그림자는 떠 있는 표면에만**: 모달 `shadow-2xl`, 토스트·드롭다운 메뉴 `shadow-lg`. 카드·페이지 요소는 테두리로만 구분한다.
  - 예외 — `Segmented` 의 선택 칸(thumb) `shadow-sm`. 흰 바탕 + 옅은 테두리만으로는 트랙(slate-100)과 구분이 약해 고른 칸이 안 보였다(2026-09-14 요청). 한 칸 위로 떠 있는 것이 곧 「선택됨」의 뜻이다.
...
- **운영 화면의 모션 자리**(2026-09-11, 재고·물류에서 시작 · 공용 컴포넌트라 다른 모듈에도 적용). 전부 CSS, 라이브러리 없음:
  ...
  - 모달 `.modal-in`: 팝업 · 확인 다이얼로그 전부. 닫힘은 언마운트 그대로.
  ...
  - 표 로딩 `TableSkeleton`(2026-09-14): 「불러오는 중이에요」 글자 대신 도착할 카드(제목 · 도구 자리 · 머리 줄 · 행)와 같은 자리의 회색 막대. **1초 안에 오면 정지 막대로 지나가고** 넘으면 `animate-pulse` — 짧은 대기에 깜빡임을 보이지 않는다. 표가 주인인 화면(재고·물류)에만 쓴다. 작업대 · 트리처럼 표가 아닌 화면에 표 모양을 그리면 도착할 모양과 달라 오히려 튄다.
  - 하지 않는 것: 숫자 카운트 전환(수량은 즉시 읽혀야 한다), 필터 결과 행 슬라이드(타이핑마다 깜빡임), 커서 따라다니는 하이라이트, `TableSkeleton` 밖의 스켈레톤.
- **AI 표면 한정 허용**: skeleton(`animate-pulse`, 대화 복원·전환 대기 — 운영 화면은 위 `TableSkeleton` 하나만), 타자 효과(새 답변 1회 — 복원된 메시지엔 쓰지 않는다), 앰비언트 배경(빈 상태에서만, `components/chat/ai-backdrop.tsx`).
...
- 금지: 바운스 오버슈트, 패럴랙스, 300ms 초과 UI 전환(위 명명 예외·앰비언트 배경 제외), AI 표면 밖의 shimmer/타자 효과/앰비언트 배경, `TableSkeleton` 밖의 skeleton.
...
| 컴포넌트 | 용도 | 비고 |
| --- | --- | --- |
| `Button` | 액션 버튼 | `primary`(블루) / `secondary` / `ghost` / `danger` × `sm/md/lg`. **sm 32px · 13px, md 36px · 14px**(「높이 사다리」). `href`를 주면 `Link`로 렌더한다 — 이동은 버튼이 아니라 링크여야 한다 |
...
| `DataTable` | 데이터 테이블 | `onRowClick`으로 행 상세·펼침 연결(행은 Tab 으로 닿고 Enter/Space 로 연다) · `emphasis`(열별) · `emphasisAt`(셀별 — 0 · — 는 내림, 미달 수량은 강조) · `rowEmphasis`(끝난 행 전부 내림) · `expandedRows` + `renderExpanded`(행 아래로 펼치는 패널 — 모달 아님, 첫 열에 ▸, `aria-expanded`/`aria-controls`, 흰 바탕 + 테두리. **여러 행이 동시에 열린다** — 하나를 열어도 다른 행을 접지 않는다) · 셀 `{ progress }`(막대 + 「6 / 10」) · `{ delta }`(증감, 「재고 증감 부호」 예외) · `colWidths`(열이 많은 상세 보기 — `table-layout: fixed` + 13px + 말줄임 + `title`). **첫 열 자동 강조는 없다** — 위치가 아니라 의미가 위계를 정한다(「위계」 절) |
...
| `TableSkeleton` | 표 로딩 | 카드 제목 · 도구 · 머리 줄 · 행 자리의 회색 막대, `role="status"`. 1초 뒤부터 깜빡임, 줄이는 모션이면 정지(「모션」 절) |
| `SettingsSection` | 설정 폼 섹션 | **카드를 쓰지 않는다** — 섹션 제목 + 그 아래 실선(`border-slate-200`), 행 사이 옅은 실선(`border-slate-100`). 설정은 값을 읽고 고치는 목록이라 상자를 겹치면 좌우 패딩이 매 섹션 들어가 담기는 행이 줄고, 왼쪽 이름과 오른쪽 버튼이 섹션마다 다른 x좌표에서 시작한다. 원시요소는 `SettingsSection`·`SettingsRows`·`SettingsRow`·`ActionRow`(이름+값 / 동작)·`FieldRow`(라벨 132px + 입력)·`SectionActions`. `desc` 는 제목 아래 한 줄 13px — 이름만으로 뜻이 안 읽히는 섹션에만(「보조 텍스트」 예외). `components/settings/settings-section.tsx` |
| `Segmented` | 2~3지 선택 | 라디오 대신 쓰는 분절 컨트롤 (없음/읽기/쓰기, 전체/부서/본인, 처리/설정, 간단/상세). **md 36px · 14px(기본) / sm 32px · 13px**(표 행 · 펼침 안). 선택된 칸은 `bg-white` + `ring-slate-300` + `shadow-sm` + 굵게, 안 고른 칸은 바탕 없이 `slate-500`. 트랙은 `bg-slate-100`. `aria-pressed`로 상태를 알린다. `label` 은 ReactNode 라 아이콘을 넣을 수 있다(재고·물류 헤더 토글). **블루를 쓰지 않는다** — 선택 상태는 액션이 아니다 |
...
| `FIELD_MD` (상수) | 36px 입력 | 카드 머리 도구 줄의 검색 펼침처럼 `Button` md · 아이콘 버튼(36)과 한 줄에 놓일 때 |
...
| `UploadReviewModal` (`components/upload-review-modal.tsx`) | 엑셀 업로드 → 확인 → 승인 | `parse`(실제 파일 읽기 — `lib/sheet.ts`: CSV 직접, .xlsx 는 `exceljs` 를 눌렀을 때만) · `classify`(행마다 갱신 · 신규 · 오류 + 사유). 오류 행은 승인에서 빠진다(부분 성공). 둘 다 없으면 옛 데모 파싱. **재고·물류 품목 마스터는 이것 대신 한 장짜리 `ItemImportModal`** 을 쓴다 |
| `ItemImportModal` (`components/inventory/item-import-modal.tsx`) | 품목 한 번에 등록하기 | 단계 표시 없는 한 장(2026-09-14) — 설명 한 줄 · 「품목 등록 양식 다운받기」 링크(.xlsx, `lib/download.ts` `downloadXlsx`) · 끌어 놓기 자리(파일 줄: 이름 · 크기 · 읽는 중 / ✓) · 「새 품목 N · 바뀌는 품목 N · 빠지는 행 N」 + 행 번호와 이유 · 바닥 [파일 찾기 / 다른 파일 올리기] [N개 등록하기]. 올린 표의 칸은 고치지 않는다 — 엑셀에서 고쳐 다시 올린다. 오류 행은 빼고 등록하며 따로 설명하지 않는다(버튼의 숫자가 말한다) |
...
보조 — 다이얼로그: `Modal`(sm~2xl · `modal.tsx`), `RecordModal`(행 상세 · `record-modal.tsx`).

**편집 폼도 팝업이다**(2026-09-18 결정). 화면을 덮는 전폭 시트(`size="screen"`)를 두었다가 발주서 작성 하나만 쓰고 걷어냈다 — 모듈 화면 위에 열리는 편집은 배경이 보이는 팝업이라야 어디서 열었는지가 남는다. 열이 많은 폼은 가장 큰 `size="2xl"`(1152px)을 쓰고, 라인은 표가 아니라 격자로 짜서 폭에 맞춰 줄을 바꾼다: xl 이상 한 줄(9열), md 두 줄(6열), 그 아래 두 열 — `min-w` 표는 폭에 맞춰 줄을 바꾸지 못해 라인 0개에도 가로 스크롤이 생긴다(2026-09-11). 머리 · 바닥은 고정, 본문만 스크롤한다(`max-h-[88vh]`). 닫기는 헤더 × 하나 — 푸터에 [닫기] 를 또 두면 확인 다이얼로그까지 「닫기」 가 넷이 된다. `Modal` 은 `document.body` 로 포털한다 — 부모에 transform 이 남아 있으면(`.fade-in`) `fixed` 가 그 부모에 갇힌다(2026-09-14). 발주서 작성의 요청자는 로그인한 구성원 이름이 자동으로 들어가는 읽기 전용 칸이다. 서식 토글 옆에는 두 서식이 실제로 찍는 열을 칩으로 나란히 보인다(「단위는 안 찍혀요」 같은 부정문 대신 무엇이 나가는지), 요약 줄(문서 N장 · 발주처별 라인 수)은 없다 — 바닥 「출력 · N장」 이 말한다.
...
| 카드 헤더 도구 (`components/inventory/card-tools.tsx`) | 왼쪽 h2, 오른쪽 [🔍 → 240px 입력창으로 펼침, 200ms] [⤓](내보내기가 필요한 표만 — 발주 현황은 없다) + 보조 버튼(secondary md). 한 줄 전부 36px. **간단/상세 전환은 없다**(2026-09-14) — 표마다 열 한 벌. 열린 검색은 아이콘이 입력창 안에 있는 한 상자, × 는 검색어가 있으면 지우기 · 없으면 접기, ESC 는 항상 접기 |
| 발주 현황의 한 줄 | **한 번의 발주서 작성**(`lib/inventory-state.ts` `orderGroups` — 관리번호 · 발주일 · 도면이 같은 발주들). 발주처가 둘 이상이면 「POWERTEC 외 1」 한 줄이고, 펼치면 발주처마다 한 덩어리(이름 · 상태 · [입고 등록] · 라인 표, 사이에 실선). 입고 칸은 막대 + 「13 / 17」(% 글자는 쓰지 않는다) |
| 발주 상태 | **셋뿐** — 「N일 경과」(리드타임을 넘긴 잔량, red strong) · 「대기」 · 「입고 완료」(2026-09-14). 부분 입고 · 등록 전 · 자체 제작은 모두 대기 — 얼마나 들어왔는지는 입고 막대가 말한다. 묶음의 상태는 가장 급한 발주를 따른다 |
| 행 펼침 | `DataTable` `expandedRows`/`renderExpanded` — 모달이 아니라 행 아래로, **여러 행을 동시에 열어 둔다**(하나를 열어도 다른 행은 접히지 않는다). 편집은 한 번에 한 발주 — 입력한 채 다른 발주의 [입고 등록] · 편집 중인 줄 접기 · [닫기] 를 누르면 「입력한 내용을 저장하지 않고 나가시겠습니까?」 [계속 입력] [저장하지 않고 나가기]. [입고 등록] 은 secondary(여러 줄이 열리면 파란 버튼이 줄마다 생긴다), 편집 중 「저장」 이 파랑 하나. **마감(「검수 완료」)은 secondary** — 잔량을 전부 취소로 만드는 길이 가장 눈에 띄는 버튼이면 안 된다 |
| 입고 등록 폼 | `<form>` — Enter 가 저장, 첫 열린 라인의 입고 칸에 포커스. **폭 960px 상한**(넓은 화면에서 품목명과 입력 칸이 멀어지지 않게), 읽는 열(발주 · 기입고 · 잔량) ‖ 적는 열(입고 80px · 판정 · 조치사항 260px) 사이 세로 실선. 잔량 0 라인은 입력 대신 「받을 수량 없음」. 「저장」 은 늘 켜 두고 빈 채로 누르면 「이번에 들어온 수량을 적어 주세요」. 초과 입고는 「잔량 N EA보다 M 많아요」로 사실을 먼저 말한다. 버튼은 폼 아래 한 줄 — 왼쪽 [잔량 전부 채우기], 오른쪽 [닫기] [검수 완료] [저장] |
| 발주번호 | 기록용 식별자다. **화면에는 두지 않는다**(2026-09-14 — 표 · 펼침 머리 · 검색 안내 전부). 출력 발주서 머리에만 남는다 — 같은 관리번호 · 같은 거래처로 재발주하면 납품서가 어느 발주서 것인지 가를 이름이 그것뿐이다. 번호로 쳐도 검색은 된다. 문서의 줄 번호(01, 02)도 출력물에만 |
| 출력 | 펼침 머리의 「발주서 출력 ▾」(`MenuButton`) — 「POWERTEC 발주서」(발주처마다) / 「26MSX-S03 OP20 전체 발주서 N장」(발주처가 둘 이상일 때). 나오는 종이 이름으로 부른다. 자체 제작은 문서가 없어 목록에 없다. 「다시 출력」처럼 이전 행동을 전제하는 말을 쓰지 않는다 |
| 설정 탭 | 표형(품목 · 거래처)은 카드 + 행 클릭 → 팝업(품목 마스터 표에는 품목 코드 · 도면 연동 배지가 없다 — 코드는 수정 팝업 · 엑셀 양식에만. 머리 버튼은 [품목 추가] · [품목 한 번에 등록하기](primary)(저장 시 자동 닫힘 + 「저장했어요」. **[저장] 은 바꾼 게 있을 때만 활성** — 수정됐는지가 버튼 상태로 보인다), 값형(안전 기준 · 문서 규칙)은 `SettingsSection` + 행마다 [수정] → 그 자리에서 고치고 [저장]. 태그처럼 작은 값 목록은 수정 모드 없이 인라인(Notion 의 다중 선택 값 편집) |
| 시스템이 모르는 것 | 말하지 않는다 — 「도착했어요」 대신 「대기」 · 「N일 경과」(리드타임 기준) |
| 품목 표기 | **어디서나 품목명 · 사양 · 규격 세 칸**(2026-09-14). 「품명」 · 「호칭」 · 「사양/시리즈」 · (한 품목을 가리키는) 「품목」을 쓰지 않고, 두세 값을 한 칸에 합치지 않는다. 순서는 품목명 → 사양 → 규격 → 수량류. 여러 품목을 한 줄로 줄이는 표(발주 현황)만 「품목」 한 칸에 「GUIDE POST 외 3」 |
| 입출고 이력 | 열 한 벌 — 일시 · 품목명 · 사양 · 규격 · 구분 · 사유 · 수량 · 조정 후 재고 · 담당자. 일시는 오늘이면 시각만(`14:20`), 이전은 `06.30 14:20`(`lib/format.ts` `formatStamp`). 사유 = 관리번호 · 조정 사유 · 메모를 「 · 」로(불합격 입고는 앞에 「불합격」). 구분은 배지 색 없이 글자, 수량은 「재고 증감 부호」 색 |
| 현재 재고 | 표는 판단할 값만 — 품목명 · 사양 · 규격 · 현재 재고 · 안전 재고 · 부족(없으면 「—」, 「미설정」 · 「충족」 글자 없음). 기초 · 입고 · 출고 · 조정은 펼침의 산식 숫자 칸 다섯 개로 한 번만. 펼침 오른쪽 [기준 바꾸기] [조정 등록](둘 다 sm secondary)이 산식 아래에 같은 크기(32)의 한 줄 폼을 연다. 펼침 이력은 입출고 이력과 같은 열(품목 세 칸만 뺌), 최근 5건 + 「이력 N건 모두 보기」 |
| 검색 | 다섯 표가 `lib/search.ts` `matchesQuery` 하나로 걸러진다 — 공백 토큰 AND, 대소문자 · 공백 · 하이픈 · 점 무시(「PO 2607」 ≈ 「PO-2607-0021」, 「guide post」 ≈ 「GUIDE POST」), 토큰은 어느 필드에 맞아도 됨(「볼트 M8」 = 품명 + 규격). 발주 표는 라인 품목명 · 사양 · 규격까지 본다. 결과 0건이면 표 안에 `DataTable` `emptyText` 한 줄(「검색 결과가 없어요」). 플레이스홀더는 실제로 보는 필드를 적는다 |

**보조 텍스트는 없다.** 제목 아래 설명 문장 · 카드 설명 · 표 각주 · 설정 행의 값 설명 전부. 남는 글자는 값 · 상태 · 행 안의 데이터 메타줄만이다.
- 예외(2026-09-14) — **이름만으로 뜻이 안 읽히는 설정 섹션의 한 줄 설명**(`SettingsSection` `desc`). 문서 규칙(관리번호 · 발주서 서식 · 가공 요청 태그)은 「이게 어디에 찍히는지」가 없으면 고칠 수 없다는 요청이었다. 설명은 무엇에 쓰이는지 한 문장 + 실제 값의 예(관리번호 미리보기의 조각별 이름)로, 툴팁이 아니라 늘 보이게. 발주서 작성의 서식 설명도 같은 이유다.
...
- **폼 다이얼로그**의 왼쪽 버튼은 **[닫기]**로 통일한다 — [취소]는 진행 중인 작업이 취소된다고 오해된다.
- **예외 — 확인 다이얼로그는 격식체다**(2026-09-11 결정). 파기 · 마감 · 삭제 · 승인처럼 되돌릴 수 없는 결정을 되묻는 `ConfirmModal` 은 제목 「~하시겠습니까?」, 본문 「~됩니다」로 쓴다. 「버릴까요?」 「지울까요?」 는 B2B 제조 시스템에서 캐주얼하고 불안하다. 토스트 · 에러 · 라벨 · 빈 상태 등 나머지 문구는 해요체 그대로.
- 확인 다이얼로그의 **거절 버튼은 거절하는 행동을 말한다** — [계속 작성] · [계속 편집] · [계속 입력] · [유지] · [돌아가기] · [연결 유지]. [닫기] 는 무엇이 닫히는지 모호하고 헤더 × 와 겹쳐 「아니오」가 둘이 된다 — 확인 다이얼로그에는 × 도 없다(`Modal closeButton={false}`). 실행 버튼은 일어날 일을 명사로(「마감」 「삭제」 「승인」 「작성 취소」 「저장하지 않고 닫기」).
- 「버리다」 「지우다」 「뿐이에요」 처럼 데이터를 쓰레기 취급하거나 수다스럽게 읽히는 어휘는 쓰지 않는다 — 「저장하지 않고 닫기」 「삭제」 「재고 27 EA보다 많이 뺄 수 없어요」.
