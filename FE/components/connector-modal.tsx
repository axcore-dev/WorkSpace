...
import { IconArrowUpRight, IconCheck, IconPlus, IconSearch, IconX } from "@/components/icons";
        ...
        <div className="mt-8">
          ...
          <dl className="mt-3 grid grid-cols-3 gap-x-6 gap-y-4 rounded-xl border border-slate-200 p-4">
            <div>
              ...
              <dd className="mt-1 text-sm text-slate-700">{connector.category}</dd>
            </div>
            <div>
              ...
              <dd className="mt-1">
                ...
                >
                  {new URL(connector.url).hostname.replace(/^www./, "")} <IconArrowUpRight size={13} />
                </a>
              </dd>
            </div>
            <div>
...
export function ConnectorModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const list = CONNECTOR_LIB.filter(
    (c) => !needle || c.name.toLowerCase().includes(needle) || c.slug.includes(needle) || c.category.includes(needle),
  );
  const groups = CONNECTOR_CATEGORIES.map((cat) => ({ cat, items: list.filter((c) => c.category === cat) })).filter(
    (g) => g.items.length > 0,
  );
    ...
    <>
      ...
      >
        <div className="thin-scroll overflow-y-auto px-5 pb-5 pt-4">
          ...
          {groups.length === 0 && (
            <p className="py-14 text-center text-sm text-slate-400">&lsquo;{q}&rsquo;에 맞는 커넥터가 없어요.</p>
          )}
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="mt-0.5 line-clamp-2 block text-xs leading-relaxed text-slate-500">{c.desc}</span>
                        </span>
                      </button>
                      <button
                        type="button"
