...
export function RefinePanel({
  ...
  workspaceId: number;
  /** 다듬을 개념들 — 행 id · 이름. 이름은 실패 줄에 쓴다 */
  targets: { rowId: number; conceptId: string; name: string }[];
  onClose: () => void;
  /** 저장이 끝났을 때. 바꾼 개념 수 · 새로 만든 수 · 지운 수 (다시 시도한 것까지 누적) */
  onApplied: (n: { updated: number; created: number; deleted: number }) => Promise<void>;
}) {
  const [phase, setPhase] = useState<"notice" | "running" | "review" | "applying">("notice");
  const [structureOnly, setStructureOnly] = useState(false);
  const [items, setItems] = useState<RefineItem[]>([]);
  ...
  async function run() {
    ...
    setRunError(null);
    try {
      const r = await refineConcepts({ workspaceId, conceptRowIds: targets.map((t) => t.rowId), structureOnly });
      setItems(r.items);
      setChecked(defaultChecks(r.items));
      setPhase("review");
    } catch (e) {
  ...
  if (phase === "notice" || phase === "running") {
    return (
      <Modal
        open
        onClose={onClose}
        title={title}
        desc={`개념 ${targets.length}개의 이름 · 동의어 · 설명 · 라벨 · 필터 · 관계를 제안받아요. 제안은 검토하고 체크한 것만 저장돼요.`}
        size="md"
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => void run()} disabled={phase === "running"}>
              {phase === "running" ? `제안 받는 중… (${targets.length}개, 1분 안쪽)` : "제안 받기"}
            </Button>
          </div>
        }
      >
        <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-[13px] leading-relaxed text-slate-700">
          <p className="font-semibold text-slate-900">고객 데이터가 어디까지 나가나요</p>
          <p className="mt-1">
            표마다 앞 2,000행을 읽어 BE 메모리에서 통계만 내고 버려요. AI 모델로 나가는 값은 <b>고유값이 30개 이하인 컬럼의 값 목록</b>(상태 · 종류 · 코드)과
            <b>숫자 · 날짜 컬럼의 최솟값 · 최댓값</b>이에요. 거래처명처럼 고유값이 많은 컬럼은 길이와 모양만 나가고, 사람 이름 · 급여 · 이메일 · 전화 · 주민번호 ·
            비밀번호로 보이는 컬럼은 아무것도 나가지 않아요.
          </p>
          <label className="mt-3 flex cursor-pointer items-start gap-2">
            <input type="checkbox" id="refine-structure-only" checked={structureOnly} onChange={(e) => setStructureOnly(e.target.checked)} className="mt-0.5" />
            <span>
              <span className="font-medium text-slate-900">구조만 보내기</span> — 값을 한 개도 보내지 않아요. 컬럼 이름 · 타입 · 주석 · 모양만으로 제안해서 라벨의 괄호(재직 · 휴직 · 퇴직) 같은 건 못 채워요.
            </span>
          </label>
        </div>
        <ul className="mt-3 max-h-40 overflow-y-auto font-mono text-[12px] text-slate-600">
          {targets.map((t) => (
            <li key={t.rowId} className="truncate py-0.5">
              {t.conceptId} <span className="text-slate-400">· {t.name}</span>
            </li>
          ))}
        </ul>
        {runError && <p className="mt-3 text-[13px] text-red-600">{runError}</p>}
      </Modal>
    );
  }
      ...
      footer={
        <div className="flex items-center gap-2">
          {applyError ? (
            ...
            <span className="mr-auto text-[12px] text-slate-500">체크 {checkedCount}개</span>
          )}
          <Button variant="secondary" onClick={onClose} disabled={phase === "applying"}>
            {applyError ? "닫기" : "건너뛰기"}
          </Button>
          <Button onClick={() => void apply()} disabled={phase === "applying" || checkedCount === 0}>
            {phase === "applying" ? "저장 중…" : applyError ? "이어서 저장" : "체크한 것 저장"}
          </Button>
        </div>
      }
    >
      <div className="thin-scroll max-h-[65vh] space-y-4 overflow-y-auto pr-1">
        {items.map((it) =>
          it.ok ? <ItemCard key={it.conceptRowId} item={it} on={on} toggle={toggle} /> : (
            <div key={it.conceptRowId} className="rounded-lg border border-slate-200 px-4 py-3 text-[13px]">
              <span className="font-mono text-slate-900">{targets.find((t) => t.rowId === it.conceptRowId)?.conceptId ?? it.conceptRowId}</span>
              <span className="ml-2 text-red-600">{it.error}</span>
            </div>
          ),
        )}
        {okItems.length === 0 && <p className="text-sm text-slate-500">제안을 받은 개념이 없어요. 다시 시도해 주세요.</p>}
      </div>
    </Modal>
  );
}
...
function ItemCard({ item, on, toggle }: { item: Extract<RefineItem, { ok: true }>; on: (k: string) => boolean; toggle: (k: string) => void }) {
  ...
  for (const a of p.aggregates) rows.push({ key: `agg.${a.id}`, field: "집계 추가", before: "—", after: <>{a.id} · {a.name} <Hint text={`미리보기 통과 · 컬럼 ${a.columns.length}`} /><span className="mt-0.5 block font-mono text-[11px] text-slate-500">{a.sql}</span></> });
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white">
      <header className="flex flex-wrap items-baseline gap-x-3 gap-y-1 border-b border-slate-100 px-4 py-2.5">
        <span className="text-[14px] font-semibold text-slate-900">{p.name || cur.name}</span>
        <span className="font-mono text-[11px] text-slate-500">{cur.conceptId}</span>
      ...
      {item.duplicates.length > 0 && (
        <p className="border-b border-slate-100 bg-amber-50 px-4 py-2 text-[12px] text-amber-700">
          같은 표를 읽는 개념이 또 있어요: <span className="font-medium">{twins}</span>. AI 가 둘 중 어느 쪽을 고를지 흔들리니 한쪽은 지워 주세요 — 아래 「표 삭제」 를 켜거나 스튜디오에서
          다른 쪽을 지우면 돼요.
        </p>
      )}
      {rows.length === 0 ? (
        <p className="px-4 py-3 text-[13px] text-slate-500">바꿀 제안이 없어요 — 지금 값이 그대로 좋아요.</p>
      ) : (
        <ul>
          <li className="grid grid-cols-[20px_96px_1fr_1fr] gap-x-3 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
            <span />
            <span>필드</span>
            <span>지금</span>
            <span>제안</span>
          ...
          {rows.map((r) => {
            ...
            return (
              <li key={r.key} className="grid grid-cols-[20px_96px_1fr_1fr] items-start gap-x-3 border-t border-slate-100 px-4 py-2 text-[13px]">
                <input type="checkbox" id={id} checked={on(k(r.key))} onChange={() => toggle(k(r.key))} className="mt-0.5" />
                <label htmlFor={id} className="cursor-pointer font-mono text-[11px] text-slate-600">
                  {r.field}
                </label>
