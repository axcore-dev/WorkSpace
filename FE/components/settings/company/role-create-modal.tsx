 ...
 * 권한(직급) 만들기 팝업.
 *
 * **권한 내용은 여기서 정하지 않는다.** 이름과 부서만 받고, 만들면 그 직급이 선택된 채로
 ...
 * 그 일을 대신한다.
 *
 * **BE 연동 seam**: `onCreate`가 직급 생성 API를 부른다. id는 지금 시각으로 만든다 —
...
export function RoleCreateModal({
  ...
  function submit() {
    ...
    onCreate({
      ...
      system: false,
      dept,
      // 아무 권한 없이 시작한다 — 새 직급이 뭘 할 수 있는지는 만든 사람이 정한다
      perms: [],
      scope: "own",
      showAmounts: false,
