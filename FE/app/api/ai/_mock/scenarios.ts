...
export function ragIngestMessage(names: string[]): ChatMessage {
  const first = names[0];
  const label =
    names.length > 1 ? `'${first}' 외 ${names.length - 1}건` : `'${first}'`;
  return {
    role: "ai",
    ...
    process: {
      ...
      trace: [
        {
          icon: "doc",
          text: `업로드 문서 텍스트 추출 (${first})`,
          result: `${names.length}건`,
        },
        {
          icon: "model",
          text: "구절(청크) 분할 — 검색 단위로 재구성",
          result: "34개 구절",
        },
        {
          icon: "data",
          text: "임베딩 생성 및 지식 인덱스 등록",
          result: "등록됨",
        },
      ],
    },
...
export function withUploadedCitation(
  msg: ChatMessage,
  docName: string,
): ChatMessage {
  return {
    ...
    sources: [
      {
        doc: docName,
        snippet: "업로드 문서 일치 구절 3건 — 지식 인덱스 유사도 0.93",
      },
      ...(msg.sources ?? []),
    ],
    process: msg.process && {
      ...
      trace: msg.process.trace && [
        {
          icon: "search",
          text: `업로드 문서 '${docName}'의 지식 인덱스에서 우선 검색`,
          result: "일치 3건",
        },
        ...msg.process.trace,
      ],
