...
#   bash INFRA/seed/seed-admin-console.sh
#
...
ADMIN_EMAIL="${SEED_ADMIN_EMAIL:-ops@axcore.ai.kr}"
ADMIN_PW="${SEED_ADMIN_PASSWORD:-Passw0rd!}"
ADMIN_NAME="김운영"
...
# 같은 이유로 질의어도 셸을 거치면 깨진다. UTF-8 파일에서 읽어 퍼센트 인코딩한다.
jget() {
  node -e "try{const o=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8'));const v=$1;console.log(v==null?'':v)}catch(e){console.log('')}" "$TMP/body"
}
...
ensure_user() {
  ...
  local code
  code=$(api POST /api/auth/signup "{\"email\":\"$email\",\"password\":\"$ADMIN_PW\",\"name\":\"$name\"}")
  case "$code" in
    201) say "가입: $email" ;;
    409) say "이미 있음: $email" ;;
    *)   echo "  [실패] 가입 $email → HTTP $code"; cat "$TMP/body"; echo; return 1 ;;
  esac
  # 개발 환경은 메일을 보내지 않는다. 토큰을 로그에서 긁는 대신 바로 확인 처리한다.
  psql_ "update shared.users set email_verified_at = coalesce(email_verified_at, now()) where email = '$email';" >/dev/null
}

login_token() {
  local email="$1"
  api POST /api/auth/login "{\"email\":\"$email\",\"password\":\"$ADMIN_PW\"}" >/dev/null
  jget "o.accessToken"
}
...
if [ "$(api GET /actuator/health)" != "200" ]; then
  echo "  [중단] BE 가 $BASE 에서 응답하지 않는다. ./gradlew bootRun 을 먼저 띄워라."
  exit 1
fi
say "BE 응답 확인"

if ! docker exec "$PG" true 2>/dev/null; then
  echo "  [중단] 컨테이너 $PG 가 없다. docker compose -f INFRA/docker-compose.db.yml up -d"
  exit 1
fi
say "DB 컨테이너 확인"
...
echo
echo "== 1. 운영자 계정"
ensure_user "$ADMIN_EMAIL" "$ADMIN_NAME" || exit 1

# 애플리케이션에 이 값을 켜는 경로가 없다. 최초 운영자는 손으로 지정하는 것이 설계다.
psql_ "update shared.users set is_internal_admin = true where email = '$ADMIN_EMAIL';" >/dev/null
say "is_internal_admin = true"

AT=$(login_token "$ADMIN_EMAIL")
if [ -z "$AT" ]; then
  echo "  [중단] 운영자 로그인 실패"
  exit 1
fi
...
COMPANIES=(
  "한빛제철 주식회사|1234567891|1101111234567|제조업|1차 철강 제조|경북 포항시 남구 철강로 12|https://hanbit-steel.co.kr|Enterprise|김구매|kim@hanbit.co.kr|054-000-0000|포항 2공장 MES 인증 정보 재발급 대기 중."
  "대성화학 주식회사|2148801232|1101112345678|제조업|기초 화학물질 제조|울산 남구 여천로 88|https://daesung-chem.co.kr|Growth|정관리|jung@daesung.co.kr|052-000-0000|"
  "서진모빌리티 주식회사|3058155511|1101113456789|제조업|자동차 부품 제조|경기 화성시 동탄산단로 7||Growth|오총무|oh@seojin.co.kr|031-000-0000|접속 링크 미개봉. 유선 확인 예정."
  "금호식품 주식회사|4128677123|1101114567890|제조업|기타 식품 제조|전남 나주시 식품로 3|https://kumho-food.co.kr|Growth|강대리|kang@kumho-food.co.kr|061-000-0000|7월분 미수금. 입금 확인 후 재개."
  "남양정밀 주식회사|5018112347|1101115678901|제조업|금속 가공기계 제조|경남 창원시 성산구 공단로 41|https://ny-precision.co.kr|Starter|윤사원|yoon@ny-precision.co.kr|055-000-0000|"
  "태양전자 주식회사|6068623450|1101116789012|제조업|전자부품 제조|충북 청주시 흥덕구 산단로 22|https://ty-electronics.co.kr|Enterprise|노팀장|noh@ty-electronics.co.kr|043-000-0000|"
  "신흥포장 주식회사|7188134560|1101117890123|제조업|골판지 상자 제조|인천 서구 검단로 9||Starter|임과장|lim@sinheung.co.kr|032-000-0000|"
)
...
for row in "${COMPANIES[@]}"; do
  ...
  body=$(node -e '
    ...
    process.stdout.write(JSON.stringify({
      ...
      website: site || undefined, plan, operatorName: "김운영",
      memo: memo || undefined,
  ...
  id=$(jget "o.id"); schema=$(jget "o.schemaName")
...
for i in 0 1 2 3; do
  ...
  link=$(jget "o.link"); token="${link##*token=}"

  ensure_user "$email" "담당자" || continue
  mtoken=$(login_token "$email")
...
echo
echo "  이제 FE 를 띄우고 위 계정으로 로그인하면 /admin 으로 들어간다."
